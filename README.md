...
**7 pull requests merged into repos I don't own, across 5 organisations** — someone else's maintainer decided each one was worth carrying:
...
- **[huggingface.js](https://github.com/huggingface/huggingface.js)** — the public `language()` lookup used `code in TABLE`, and `in` walks the prototype chain, so `language("toString")` returned `Object.prototype.toString` — a function — out of a signature typed `Language | null`. The existing invalid-code test passed only because its input wasn't on `Object.prototype`, so the contract looked covered and wasn't. Fixed with `Object.hasOwn` and a regression test over the inherited names → [#2320](https://github.com/huggingface/huggingface.js/pull/2320)
...
Open and in review: a worker-lifecycle fix in `@huggingface/hub` where a completed hash left its abort listener attached, so a later abort terminated a worker already returned to the pool ([huggingface.js#2325](https://github.com/huggingface/huggingface.js/pull/2325)), and a release run-ledger binding build artifacts to their gate decisions in openmed ([#1899](https://github.com/maziyarpanahi/openmed/pull/1899)).
