...
Nine problem domains — HTTP framing, DNS zone files, DICOM medical imaging, medical NLP, RSS parsing,
supply-chain signing, package metadata, an L1 bridge, and a local-AI IDE — across **Python, Rust, TypeScript
and Solidity**.
...
1. **Screen for the gap.** Rank a language's most-depended-on parser, encoding, network and crypto packages
   ...
   target. ⚠️ "No `fuzz/` directory" is *not* the same as unfuzzed — several are fuzzed downstream by a
